...
t_data <- raw_csv %>%
  mutate (
    ...
    opts_ndp = factor(opts_ndp, levels = c(0, 0.17, 0.33, 0.50, 0.67, 0.83, 1.0, 1.25), labels = c("0%", "1/6", "1/3", "1/2", "2/3", "5/6", "100%", "AoS")),
...
gplt <- ggplot(t_data %>% filter(rps_offset < 0.05),
    aes(x = avg_rps_avg_rps, group = opts_ndp, color = opts_ndp)) +
  ...
  scale_y_log10(breaks = log_breaks(n=10), label = scales::label_comma(accuracy = 0.1), expand = c(0.01,0)) +
...
```{r table_network}
net_table <- t_data %>% filter(opts_req_rps <= 150) %>%
  group_by(opts_ndp) %>%
...
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_computenet_tx_bytes_sum + monitor_storagenet_tx_bytes_sum) / results_count, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
...
gplt <- ggplot(t_data %>% filter(rps_offset < 0.05),
  ...
  coord_cartesian(ylim = c(0, 100)) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(aes(linetype="Compute", y = 100.0*(monitor_computecpums_idle_sum)/(monitor_computecpums_user_sum + monitor_computecpums_nice_sum + monitor_computecpums_system_sum + monitor_computecpums_idle_sum + monitor_computecpums_iowait_sum + monitor_computecpums_irq_sum + monitor_computecpums_softirq_sum))) +
  geom_line(aes(linetype="Storage", y = 100.0*(monitor_storagecpums_idle_sum)/(monitor_storagecpums_user_sum + monitor_storagecpums_nice_sum + monitor_storagecpums_system_sum + monitor_storagecpums_idle_sum + monitor_storagecpums_iowait_sum + monitor_storagecpums_irq_sum + monitor_storagecpums_softirq_sum))) +
  scale_x_log10(breaks = log_breaks(n=10), label = scales::label_comma(accuracy = 0.1)) +
  scale_y_continuous(breaks = pretty_breaks(n=10), label = scales::label_comma(accuracy = 1)) +
  labs(title = "CPU load", color = "% NDP", shape = "% NDP", linetype = "Node") +
  xlab("Throughput [requests/second]") + ylab("Cpu usage (%)")
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(linetype="Compute",x = opts_req_rps, y = monitor_computeprocs_running_avg, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(linetype="Storage",x = opts_req_rps, y = monitor_storageprocs_running_avg, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(linetype="Compute", x = opts_req_rps, y = monitor_computefaasm_started_avg, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(linetype="Storage", x = opts_req_rps, y = monitor_storagefaasm_started_avg, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(linetype="Compute", x = opts_req_rps, y = monitor_computefaasm_waiting_queued_avg, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(linetype="Storage", x = opts_req_rps, y = monitor_storagefaasm_waiting_queued_avg, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
```{r plot_cpu_softirq}
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_computecpums_softirq_sum / total_requests, group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_storagecpums_softirq_sum / total_requests, group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
ggplot(t_data %>% filter(opts_req_rps>50)) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_computecswitches_sum / (opts_req_time), group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_storagecswitches_sum / (opts_req_time), group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
ggplot(t_data %>% filter(opts_req_rps>50)) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_computecswitches_sum / (total_requests), group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_storagecswitches_sum / (total_requests), group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
...
ggplot(t_data %>% filter(opts_req_rps>30)) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_computeload_avg1_avg, group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_storageload_avg1_avg, group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity') +
  labs(title = "Load") +
...
```{r plot_cpu_ms}
ggplot(t_data %>% filter(opts_req_rps>30)) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_computecpums_user_sum + monitor_computecpums_system_sum + monitor_computecpums_nice_sum) / (results_count), group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_storagecpums_user_sum + monitor_storagecpums_system_sum + monitor_storagecpums_nice_sum) / (results_count), group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity') +
...
```{r plot_cpu_idle_ms}
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_computecpums_idle_sum) / total_requests, group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_storagecpums_idle_sum) / total_requests, group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
...
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_computemem_active_avg - cmembase, group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
...
smembase <- min(t_data$monitor_storagemem_active_min)
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_storagemem_active_avg - smembase, group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = pretty_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  labs(title = "Average Active memory per request over baseline") +
  xlab("Req/s") + ylab("bytes")
...
```{r plot_net_tx_bytes}
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_computenet_tx_bytes_sum / total_requests, group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = monitor_storagenet_tx_bytes_sum / total_requests, group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
...
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_computenet_tx_bytes_sum + monitor_storagenet_tx_bytes_sum) / total_requests, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1), expand = c(0.01, 0.01)) +
  scale_y_continuous(trans = 'identity', expand = c(0.1,0), limits = c(0, NA), label = scales::label_comma(accuracy = 1)) +
  labs(title = "Total bytes per request sent over network") +
  xlab("Req/s") + ylab("bytes/request")
...
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = opts_req_rps, y = (monitor_computenet_tx_bytes_sum + monitor_storagenet_tx_bytes_sum) / (opts_req_time), group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
...
```{r plot_xverify_rps}
ggplot(t_data) +
  geom_point(stat = 'identity', aes(x = opts_req_rps, y = avg_rps_avg_rps, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp), shape=opts_ndp)) +
