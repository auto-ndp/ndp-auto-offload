...
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_computemem_active_avg, group = interaction(opts_function,opts_ndp), color = interaction('compute',opts_function,opts_ndp))) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_storagemem_active_avg, group = interaction(opts_function,opts_ndp), color = interaction('storage',opts_function,opts_ndp))) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
...
```{r plot_xverify_rps}
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = opts_req_rps, group = interaction(opts_function,opts_ndp), color = interaction(opts_function,opts_ndp))) +
