...
gplt <- ggplot(t_data %>% filter(opts_req_rps == 1),
...
lat_table <- t_data %>% filter(opts_req_rps == 1) %>%
  select(opts_ndp,
...
```{r table_network}
net_table <- t_data %>% filter(opts_req_rps <= 10) %>% filter(opts_req_rps >= 1) %>%
  group_by(opts_ndp) %>%
...
ggplot(t_data %>% filter(opts_req_rps>1)) +
