...
t_data <- raw_csv %>%
  mutate (
    ...
    opts_ndp = factor(opts_ndp, levels = c(0, 0.25, 0.33, 0.50, 0.67, 0.75, 1.0, 1.25), labels = c("0%", "25%", "33%", "50%", "67%", "75%", "100%", "AoS")),
...
ggplot(t_data) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = (monitor_computenet_tx_bytes_sum + monitor_storagenet_tx_bytes_sum) / results_count, group = opts_ndp, color = opts_ndp)) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1), expand = c(0.01, 0.01)) +
  scale_y_continuous(trans = 'identity', breaks = pretty_breaks(n=10), label = scales::label_comma(accuracy = 1)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(linetype="Compute",x = avg_rps_avg_rps, y = monitor_computeprocs_running_avg, group = opts_ndp, color = opts_ndp)) +
  geom_line(stat = 'identity', aes(linetype="Storage",x = avg_rps_avg_rps, y = monitor_storageprocs_running_avg, group = opts_ndp, color = opts_ndp)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(linetype="Compute", x = avg_rps_avg_rps, y = monitor_computefaasm_started_avg, group = opts_ndp, color = opts_ndp)) +
  geom_line(stat = 'identity', aes(linetype="Storage", x = avg_rps_avg_rps, y = monitor_storagefaasm_started_avg, group = opts_ndp, color = opts_ndp)) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(linetype="Compute", x = avg_rps_avg_rps, y = monitor_computefaasm_waiting_queued_avg, group = opts_ndp, color = opts_ndp)) +
  geom_line(stat = 'identity', aes(linetype="Storage", x = avg_rps_avg_rps, y = monitor_storagefaasm_waiting_queued_avg, group = opts_ndp, color = opts_ndp)) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_computecswitches_sum / (opts_req_time), group = opts_ndp, color = opts_ndp, linetype = 'Compute')) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_storagecswitches_sum / (opts_req_time), group = opts_ndp, color = opts_ndp, linetype = 'Storage')) +
  scale_x_continuous(trans = 'identity', breaks = breaks_pretty(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = breaks_pretty(n=12)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_computecswitches_sum / (total_requests), group = opts_ndp, color = opts_ndp, linetype = 'Compute')) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_storagecswitches_sum / (total_requests), group = opts_ndp, color = opts_ndp, linetype = 'Storage')) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_computeload_avg1_avg, group = opts_ndp, color = opts_ndp, linetype = 'Compute')) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_storageload_avg1_avg, group = opts_ndp, color = opts_ndp, linetype = 'Storage')) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = pretty_breaks(n = 10)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = (monitor_computecpums_user_sum + monitor_computecpums_system_sum + monitor_computecpums_nice_sum) / (results_count), group = opts_ndp, color = opts_ndp, linetype = 'Compute')) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = (monitor_storagecpums_user_sum + monitor_storagecpums_system_sum + monitor_storagecpums_nice_sum) / (results_count), group = opts_ndp, color = opts_ndp, linetype = 'Storage')) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity') +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = (monitor_computecpums_user_sum + monitor_computecpums_system_sum + monitor_computecpums_nice_sum + monitor_storagecpums_user_sum + monitor_storagecpums_system_sum + monitor_storagecpums_nice_sum) / (results_count), group = opts_ndp, color = opts_ndp)) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = pretty_breaks(n=10)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_computemem_active_avg - !!cmembase, group = opts_ndp, color = opts_ndp, linetype = 'Compute')) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_storagemem_active_avg - !!smembase, group = opts_ndp, color = opts_ndp, linetype = 'Storage')) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  scale_y_continuous(trans = 'identity', breaks = pretty_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
  labs(title = "Average Active memory per request over baseline") +
  xlab("Req/s") + ylab("bytes")
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_computenet_tx_bytes_sum / total_requests, group = opts_ndp, color = opts_ndp, linetype = 'Compute')) +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = monitor_storagenet_tx_bytes_sum / total_requests, group = opts_ndp, color = opts_ndp, linetype = 'Storage')) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = (monitor_computenet_tx_bytes_sum + monitor_storagenet_tx_bytes_sum) / total_requests, group = opts_ndp, color = opts_ndp)) +
  scale_x_continuous(trans = 'log', breaks = log_breaks(n=20), label = scales::label_comma(accuracy = 1), expand = c(0.01, 0.01)) +
...
ggplot(t_data) +
  paper_disc_scale + paper_fill_scale + paper_shape_scale + paper_theme +
  geom_line(stat = 'identity', aes(x = avg_rps_avg_rps, y = (monitor_computenet_tx_bytes_sum + monitor_storagenet_tx_bytes_sum) / (opts_req_time), group = opts_ndp, color = opts_ndp)) +
