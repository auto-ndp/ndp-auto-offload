...
gplt <- ggplot(t_data %>% filter(rps_offset < 0.05),
  ...
  scale_x_log10(breaks = log_breaks(n=10), label = scales::label_comma()) +
  scale_y_log10(breaks = log_breaks(n=10), label = scales::label_comma(), expand = c(0.01,0)) +
...
gplt <- ggplot(t_data %>% group_by(ndp_percent) %>% summarise(max(avg_rps_avg_rps), first(ndp_factor)),
  ...
  scale_y_continuous(breaks = breaks_pretty(n=10), label = scales::label_comma(), expand = c(0,0), limits=c(0,max(t_data$avg_rps_avg_rps * 1.05))) +
  labs(title = "Requests per second achieved", shape = "% NDP") +
  xlab("Fraction offloaded [%]") + ylab("Maximum throughput [requests/second]")
